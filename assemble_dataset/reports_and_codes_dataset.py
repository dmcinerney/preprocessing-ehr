...
import random
from assemble_dataset.dataset import Patient, MedicalDataset
from utils import to_file

class ReportsToCodesAbstract(MedicalDataset):
    @classmethod
...
class ReportsToCodes(ReportsToCodesAbstract):
    @classmethod
    def get_datapoints(cls, patient_id, counts, reports, codes, code_mapping=None, code_graph=None, frequency_threshold=2):
...
def main():
    ...
    os.mkdir(new_folder)
    train_dataset = ReportsToCodes.create(patient_ids[:div1], reports=reports, codes=codes, code_mapping=code_mapping, code_graph=code_graph)
    ...
    val_dataset = ReportsToCodes.create(patient_ids[div1:div2], reports=reports, codes=codes, code_mapping=code_mapping, code_graph=code_graph)
    ...
    test_dataset = ReportsToCodes.create(patient_ids[div2:], reports=reports, codes=codes, code_mapping=code_mapping, code_graph=code_graph)
